            ...
            <table class="data" id="question" border="1">
                ...
                @foreach($child_questions as $child_question)
                    <tr id="{{$child_question->id}}">
                        ...
                        <td>
                            <a href="{{route('user.edit',$child_question->id)}}"><button class="btn btn-primary edit" role="edit"><i class="material-icons" data-target="#deleteModal">edit</i> Edit</button></a>
                            <a href="" onclick="return false;"><button class="btn btn-danger delete" route="{{route('user.destroy',['id'=>$child_question->id])}}" role="delete" data-target="#deleteModal"><i class="material-icons">delete</i> Delete</button></a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
